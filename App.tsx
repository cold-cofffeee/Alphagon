...
// Fix: Added missing PLATFORM_ICONS import
...
const App: React.FC = () => {
  ...
  const [adminSection, setAdminSection] = useState<'dashboard' | 'users' | 'ai' | 'logs'>('dashboard');
    ...
    <div className="w-64 bg-zinc-900/50 border-r border-zinc-800 p-6 flex flex-col h-full sticky top-20">
      <div className="space-y-1">
        <button onClick={() => setAdminSection('dashboard')} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${adminSection === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white hover:bg-zinc-800'}`}>
          ...
          <span className="text-sm font-bold uppercase tracking-widest">Command</span>
        </button>
        ...
        <button onClick={() => setAdminSection('ai')} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${adminSection === 'ai' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white hover:bg-zinc-800'}`}>
          ...
          <span className="text-sm font-bold uppercase tracking-widest">AI Systems</span>
        </button>
      ...
      <div className="mt-auto border-t border-zinc-800 pt-6">
        <div className="bg-zinc-800 rounded-xl p-4">
          <p className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-widest mb-2">Service Status</p>
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-green-500 rounded-full" />
            <span className="text-xs text-white font-medium tracking-tight">System Operational</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] overflow-hidden">
        <table className="w-full text-left">
          <thead>
            <tr className="bg-zinc-800/30 text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">
          ...
          <tbody className="divide-y divide-zinc-800">
            {adminUsers.map(u => (
              <tr key={u.id} className="hover:bg-white/[0.02] transition-colors group">
                <td className="px-6 py-5">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-600 font-medium">ID: {u.id}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-5">
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center space-x-2">
                    <span className="text-sm font-bold text-white">{u.credits.toLocaleString()}</span>
                    <button onClick={() => {}} className="text-zinc-500 hover:text-indigo-400"><Plus className="w-3 h-3" /></button>
                  </div>
                </td>
                <td className="px-6 py-5 font-mono text-xs text-zinc-500">{(u.tokensConsumed / 1000).toFixed(1)}k</td>
                <td className="px-6 py-5 text-right">
                  <div className="flex items-center justify-end space-x-2">
                    <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-white"><Settings className="w-4 h-4" /></button>
                    <button className={`p-2 hover:bg-zinc-800 rounded-lg ${u.status === 'Active' ? 'text-zinc-400 hover:text-red-400' : 'text-red-400 hover:text-green-500'}`}>
  ...
  const AdminAIView = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-display font-bold text-white tracking-tight">Intelligence Orchestrator</h2>
          <p className="text-zinc-500 text-sm">Control underlying models and heuristic thresholds.</p>
        </div>
      </div>
        ...
        {aiConfigs.map(config => (
          <div key={config.id} className="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-8 space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="p-3 bg-zinc-800 rounded-2xl text-indigo-400">
                  {PLATFORM_ICONS[config.id]}
                </div>
                <div>
                  <h4 className="text-lg font-bold text-white tracking-tight">{config.id}</h4>
                  <div className="flex items-center space-x-2">
                    <div className="w-1.5 h-1.5 bg-green-500 rounded-full" />
                    <span className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-widest">Active System</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center bg-black/40 p-1 rounded-xl border border-zinc-800">
                <button 
                  onClick={() => setAiConfigs(prev => prev.map(c => c.id === config.id ? {...c, model: 'gemini-3-flash-preview'} : c))}
                  className={`px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-widest rounded-lg transition-all ${config.model === 'gemini-3-flash-preview' ? 'bg-indigo-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >Flash</button>
                <button 
                  onClick={() => setAiConfigs(prev => prev.map(c => c.id === config.id ? {...c, model: 'gemini-3-pro-preview'} : c))}
                  className={`px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-widest rounded-lg transition-all ${config.model === 'gemini-3-pro-preview' ? 'bg-indigo-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >Pro</button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-widest block mb-2">System Instruction Override</label>
                <textarea 
                  className="w-full h-32 bg-black/30 border border-zinc-800 rounded-2xl p-4 text-xs text-zinc-400 focus:outline-none focus:border-indigo-500 transition-all font-mono"
                  value={config.systemPrompt}
                  onChange={(e) => setAiConfigs(prev => prev.map(c => c.id === config.id ? {...c, systemPrompt: e.target.value} : c))}
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-black/30 border border-zinc-800 p-4 rounded-2xl">
                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase tracking-widest block mb-1">Credit Cost</label>
                  <input type="number" value={config.creditCost} className="bg-transparent text-white font-bold text-lg w-full focus:outline-none" />
                </div>
                <div className="bg-black/30 border border-zinc-800 p-4 rounded-2xl">
                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase tracking-widest block mb-1">Risk Threshold</label>
                  <input type="number" value={config.safetyThreshold} className="bg-transparent text-white font-bold text-lg w-full focus:outline-none" />
                </div>
              </div>
            </div>
          ...
          {view === 'history' ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setHistory([])} className="flex items-center space-x-2 text-zinc-500 hover:text-red-400 transition-colors text-sm font-medium px-4 py-2 hover:bg-red-500/10 rounded-lg">
            ...
            <div className="flex flex-col items-center justify-center min-h-[75vh] space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
              <div className="text-center space-y-6 max-w-3xl">
                <div className="inline-flex items-center space-x-2 px-3 py-1 bg-zinc-900 border border-zinc-800 rounded-full text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-4">
                  <Zap className="w-3 h-3 text-indigo-400" /><span>On-Demand Creative Workspace</span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in fade-in zoom-in-95 duration-500">
              <aside className="lg:col-span-3 space-y-8">
                <section className="bg-zinc-900/40 border border-zinc-800 rounded-[2rem] p-8 space-y-8 shadow-2xl sticky top-24">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 text-indigo-400"><Settings2 className="w-5 h-5" /><h3 className="font-display font-bold text-lg uppercase tracking-tight">Context Layer</h3></div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {EMOTIONS.map(e => <button key={e} onClick={() => setSettings(s => ({...s, emotion: e}))} className={`px-3 py-2 text-[11px] font-bold rounded-xl border transition-all ${settings.emotion === e ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg' : 'bg-zinc-800 border-zinc-700 text-zinc-500 hover:border-zinc-500'}`}>{e}</button>)}
                      </div>
                    </div>
                    <div className="space-y-4">
                      <label className="text-[10px] uppercase tracking-[0.15em] text-zinc-500 font-extrabold block">Preferences</label>
                      <select value={settings.tone} onChange={e => setSettings(s => ({...s, tone: e.target.value}))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none appearance-none cursor-pointer">{TONES.map(t => <option key={t}>{t}</option>)}</select>
                      <select value={settings.language} onChange={e => setSettings(s => ({...s, language: e.target.value}))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none appearance-none cursor-pointer">{LANGUAGES.map(l => <option key={l}>{l}</option>)}</select>
                      <select value={settings.region} onChange={e => setSettings(s => ({...s, region: e.target.value}))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none appearance-none cursor-pointer">{REGIONS.map(r => <option key={r}>{r}</option>)}</select>
                ...
                {insights && (
                  <div className="space-y-6">
                    <section className="bg-indigo-950/20 border border-indigo-500/30 rounded-3xl p-7 space-y-4 shadow-xl">
                      <div className="flex items-center space-x-2 text-indigo-400"><BarChart3 className="w-5 h-5" /><h3 className="font-display font-bold text-lg uppercase tracking-tight">Market Intel</h3></div>
